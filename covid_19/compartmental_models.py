...
def _lossSIR(params, confirmed_cases, recovered_cases, y0):
    beta, gamma = params
    ...
    def _SIR(t, y):
        ...
        N = S + I + R
        return [
            -beta*S*I/N,                        # dS/dt - change of susceptibles
            beta*S*I/N - gamma*I,               # dI/dt - change of infectious 
            gamma*I                            # dR/dt - change of recovered/removed/dead/quarantineed
            ]
    sol = solve_ivp(_SIR, (0, size), y0, method='RK45', t_eval=np.arange(0, size, 1), vectorized=True)
...
def _lossSEIR(params, confirmed_cases, recovered_cases, y0):
    ...
    def _SEIR(t, y):
        ...
        N = S + E + I + R
        return [
            -beta*S*I/N - delta*S*E,            # dS/dt - change of susceptibles
    ...
    sol = solve_ivp(_SEIR, (0, size), y0, method='RK45', t_eval=np.arange(0, size, 1), vectorized=True)
...
class SIR(object):
    ...
    def fit(self, confirmed_cases, recovered_cases, initial_conditions):
        ...
        opt = minimize(_lossSIR, [0.001, 0.001],
...
class SEIR(object):
    ...
    def fit(self, confirmed_cases, recovered_cases, initial_conditions):
        self.y0 = initial_conditions
        opt = minimize(_lossSEIR, [0.001, 0.001, 0.001, 0.001],
                       args=(confirmed_cases, recovered_cases, self.y0),
                       method='L-BFGS-B',
    ...
    def predict(self, n_days):
        def SEIR(t, y):
            ...
            N = S + E + I + R
            return [
                -self.beta*S*I/N - self.delta*S*E,
